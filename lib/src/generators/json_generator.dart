...
/// Result of JSON generation operation.
///
/// Contains information about the generated JSON localization files including
/// the number of files created, total keys, and missing translations.
class JsonResult {
  /// Number of JSON files successfully created.
...
/// Generates JSON localization files from CSV for custom parsing.
///
/// This generator reads a translated CSV file and creates JSON files
/// with simple key-value pairs. Useful for custom localization solutions
/// or third-party packages like easy_localization.
///
/// Example usage:
/// ```dart
...
class JsonGenerator {
  ...
  /// Generate JSON localization files from a CSV
  ...
  /// [outputDir]      - folder to write JSON files into (e.g. assets/translations)
  static Future<JsonResult> generate({
    ...
    for (final entry in translations.entries) {
      ...
      // Generate JSON file
      final fileName = '$langCode.json';
      final filePath = '$outputDir/$fileName';
      final content = _generateJsonFile(keyValues);
  ...
  static String _generateJsonFile(Map<String, String> translations) {
    ...
    for (int i = 0; i < entries.length; i++) {
      ...
      final key = entry.key;
      final value = _escapeJsonString(entry.value);
  ...
  static String _escapeJsonString(String value) {
    return value
  ...
  /// Write json_report.txt if there are any missing translations
  static Future<void> writeReport({
    required JsonResult result,
    required String outputDir,
    void Function(String)? onProgress,
  }) async {
    ...
    buf.writeln('📊 JSON Generation Report');
    buf.writeln('Generated: $dateStr');
    buf.writeln('Total keys: ${result.totalKeys}');
    buf.writeln('JSON files created: ${result.filesCreated}');
    buf.writeln(
    ...
    final reportPath = '$outputDir/json_report.txt';
